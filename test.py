...
# Define the dataset
test_dataset = get_dataset(args['test_path'], test_mask_path=args['test_mask_1~60_path'], is_train=False,
                           image_size=args['image_size'])  # 注释
...
if os.path.exists(args['test_ckpt']):
    ...
    print(c.blue('------------------------------------------------------------------'))
    print('resume training with iterations: ' + c.yellow(str(iterations)) + ', previous_epoch: ' + c.yellow(
        str(previous_epoch)))
    print(c.blue('------------------------------------------------------------------'))
else:
...
# test_loader = DataLoader(test_dataset, shuffle=False, pin_memory=True,
#                          batch_size=args['batch_size'] // args['world_size'],
#                          num_workers=args['num_workers']) #删除num_workers
