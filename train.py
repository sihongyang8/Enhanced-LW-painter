...
from losses.cr import ContrastLoss
...
# open config file
with open('./config/model_config.yml', 'r',encoding='UTF-8') as config:
    args = yaml.load(config,Loader=yaml.SafeLoader)
...
for epoch in range(args['train_epoch']):
    ...
    for it, items in enumerate(tqdm(loader, disable=False)):
        ...
        contrast_loss = contras.forward(pred_img,items['img'],items['mask_img'])



        G_loss = args['Lambda_L1'] * L1_loss + args['Lambda_Perceptual'] * Perceptual_loss + args['Lambda_LG'] * LG + \
                 args['Lambda_LHSV'] * LHSV + args['Lambda_Style'] * style_loss +args['Lambda_contrast'] * contrast_loss
        G_loss.backward()
        optimizer.step()
        ...
        if iterations % 500 == 0:
            ...
            writer.add_scalar('loss/contrast_loss', contrast_loss.item(), iterations)
    ...
    logger.info(
        ...
        f"L1_loss {L1_loss.item():.5f} | Perceptual_loss {Perceptual_loss.item():.5f}. | style_loss {style_loss.item():.5f}. | HSV_Total_loss {LHSV.item():.5f} | . | con_Total_loss {contrast_loss.item():.5f}. |"
